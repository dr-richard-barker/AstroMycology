...
async function buildEntry(item: { name: string; blob: Blob }, sidecar?: Record<string, string>): Promise<{ meta: EntryMeta; blob: Blob }> {
  ...
  let title = item.name;
  let mediaKind: 'photo' | 'thermal' = /thermal|_ir[_.]|_fir[_.]/i.test(item.name) ? 'thermal' : 'photo';

  // exifr reads HEIC EXIF straight from the original bytes, but the browser
  // can't decode HEIC pixels — that needs a separate WASM decode below, only
  // for compression/display.
  let exif: any = null;
  ...
  if (sidecar) {
    ...
    for (const [k, v] of Object.entries(sidecar)) {
      ...
      if (/^(media_?kind|sensor|image_?type)$/i.test(k) && /^(photo|thermal)$/i.test(v)) { mediaKind = v.toLowerCase() as 'photo' | 'thermal'; continue; }
...
export async function loadLocalEntries(slug: string): Promise<Ec5Entry[]> {
  ...
  for (const em of rec.entries) {
    ...
    out.push({
      ...
      photoUrl: url, thumbUrl: url, mediaKind: em.mediaKind,
